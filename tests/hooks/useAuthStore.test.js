...
describe('Pruebas en useAuthStore', () => {
  ...
  test('startLogin debe realizar el login correctamente', async () => {
    ...
    expect({ errorMessage, status, user }).toEqual({
      ...
      user: { name: 'Ruben', uid: '65b55a9150e1c776af4de967' }
    });
  ...
  test('startLogin debe fallar en la autenticación', async () => {
    ...
    await act(async () => {
      await result.current.startLogin({ email: "cualquiera@gmail.com", password: "123456789" });
    });
    const { errorMessage, status, user } = result.current;
    ...
    waitFor(() => (result.current.errorMessage).toBe(undefined));

  });

  test('startRegister debe crear un usuario', async () => {

    const newUser = { email: "noexiste@gmail.com", password: "123456789", name: "Test User" };
    ...
    expect({ errorMessage, status, user }).toEqual({
      errorMessage: undefined,
      status: 'authenticated',
      user: { name: 'Test User', uid: '12345670' }
    });
